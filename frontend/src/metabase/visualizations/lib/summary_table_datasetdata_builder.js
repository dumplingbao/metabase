...
const buildColumnHeaders = (//生成列标题
  {
    groupsSources,
    columnsSource,
    valuesSources,
  ...
  const partGroupingsRaw = groupsSources
    ...
    .map(toColumnHeader);//行字段
  const partValuesRaw = valuesSources
    .map(getColumn)
    .map(setSource(AGGREGATION))
    .map(toColumnHeader);//值

  const isPivoted = columnsSource.length > 0;//列字段
  if (isPivoted) {
